...
import {indentedString, getSymbol, isDefined, taskNumber} from './utils.js';
...
const fullRenderer = (tasks, options, level = 0) => {
    ...
    for (const task of tasks) {
        ...
        // Main output
        output.push(indentedString(`${getSymbol(task, options)} ${task.title}${skipped}`, level));
...
const summaryRenderer = (tasks, options, level = 0) => {
    ...
    tasks.forEach((task, index) => {
        if (task.hasFailed()) {
            ...
            output.push(indentedString(`${getSymbol(task, options)} ${taskNumber(index, tasks)}: ${task.title} - ${task.output}`, level));
        } else {
            // Handle any task data that needs to be output
            if (isDefined(task.output)) {
                let data = dataRenderer(task.output, taskNumber(index, tasks));
                if (isDefined(data)) {
                    output.push(indentedString(data, level));
                }
            }
        }
        if (task.isPending()) {
            output.push(indentedString(`${getSymbol(task, options)} ${taskNumber(index, tasks)}: ${task.title}`, level));
        }
    ...
    output.push(indentedString(`Total: ${tasks.length}. Complete: ${states.complete.length}, Failed: ${states.failed.length}, Skipped: ${states.skipped.length}, Disabled: ${states.disabled.length}`, level));
