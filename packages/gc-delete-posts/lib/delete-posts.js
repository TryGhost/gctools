...
export const name = 'Delete posts';

// The command to run and any params
export const flags = 'delete-posts <apiURL> <adminAPIKey>';

// Description for the top level command
export const desc = 'Delete a filtered set of posts';
...
export const run = async (argv) => {
    ...
    const confirmChanges = (!argv.confirm && !isInteractive) || await prompts.confirm({
        message: `Do you want to delete ${chalk.red(thePostsCount)} posts based on the filter ${chalk.red(thePostsFilter)} ?`
    });
    ...
    const deletedPosts = await deletePosts({
        api: argv.api,
        type: 'posts',
        items: thePosts,
        verbose: argv.verbose
    ...
    ui.log.ok(`${deletedPosts.length} posts were deleted in ${Date.now() - timer}ms.`);
