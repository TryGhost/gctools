...
async function run() {
        ...
        sourceAnswers = await inquirer.prompt([
            {
                ...
                validate: (input) =>
                    input.trim().length > 0 || 'Please provide a path to a CSV file',
            },
        ]);
    }
        ...
        const labelInput = await inquirer.prompt([
            {
                ...
                filter: (val) => val.trim(),
                validate: (input) =>
                    input.trim().length > 0 || 'Please provide a label for group A',
            },
            {
                type: 'input',
                name: 'labelB',
                ...
                filter: (val) => val.trim(),
                validate: (input) =>
                    input.trim().length > 0 || 'Please provide a label for group B',
            },
            {
                type: 'number',
                name: 'chunkSize',
    ...
    if (source === 'api') {
        ui.log.info(
            `Input: ${chalk.bold.blue('Ghost Admin API')}${answers.filter ? ` (filter: ${chalk.yellow(answers.filter)})` : ''}`,
        );
    } else {
        ui.log.info(`Input: ${chalk.bold.blue(answers.csvPath)}`);
    }
    ui.log.info(
        `Output: ${chalk.bold.blue(answers.output)}/${chalk.bold.yellow(answers.baseName)}-{all,a,b}.csv`,
    );

    if (addLabels) {
        ui.log.info(
            `Labels: A → ${chalk.bold.yellow(answers.labelA)}, B → ${chalk.bold.yellow(answers.labelB)} (chunks of ${answers.chunkSize})`,
        );
    }
    ...
    if (addLabels && context.labelled) {
        ui.log.ok(
            `Labelled group A "${answers.labelA}" (${context.labelled.a}) and group B "${answers.labelB}" (${context.labelled.b}).`,
        );
    }
