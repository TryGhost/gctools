...
const options = [
    ...
    },
    {
        ...
        name: 'addAuthor',
        message:
            'Create a dummy author? (requires a staff access token; an integration key will be skipped with importable JSON)',
        default: false,
    },
    {
        type: 'input',
    ...
    },
    {
        ...
        name: 'nav',
        message:
            'Update the navigation menu? (requires a staff access token; an integration key will be skipped with paste-able JSON)',
        default: true,
    },
];
...
async function run() {
    await inquirer.prompt(options).then(async (answers) => {
        ...
        try {
            ...
            ui.log.ok(
                `Seeded ${s.posts} posts${s.aboutPage ? ', /about page' : ''}${s.styleGuide ? ', style guide' : ''}${s.author ? ', author' : ''}${s.navigation ? ', navigation' : ''} in ${Date.now() - timer}ms.`,
            );
        } catch (error) {
