    ...
    return [
        initialise(options),
        {
            ...
            task: async (ctx, task) => {
                ...
                ctx.tagPool = buildTagPool({
                    extraTags: ctx.args.tags === false ? 0 : ctx.args.extraTags,
                });
                ...
                if (ctx.args.dryRun) {
                    ...
                    ctx.assetManager = createAssetManager({
                        api: ctx.api,
                        args: ctx.args,
                        tmpRoot: ctx.tmpRoot,
                    });
        ...
        },
        {
            ...
            task: async (ctx, task) => {
                ...
                try {
                    const existing = await findExistingAuthor({
                        api: ctx.api,
                        slug: author.slug,
                        email: author.email,
                    });
                    ...
                    if (error.code === AUTHOR_NO_PERMISSION) {
                        ctx.warnings.push({
                            type: 'author',
                            message:
                                'Author could not be created automatically. Integration API keys cannot import staff users — re-run with an owner/admin staff access token to enable this, or import the JSON below via Ghost Admin > Settings > Labs > Import content.',
                            importFile: buildImportFile(author),
                        });
                        task.skip(
                            'Integration key cannot create authors (use a staff access token)',
                        );
                    } else {
        ...
        },
        {
            ...
            task: async (ctx, task) => {
                ...
                for (const index of indices) {
                    const authors =
                        ctx.dummyAuthor && primaryIndices.has(index)
                    ...
                    if (ctx.args.dryRun) {
                        ...
                        const when = schedule[index]
                            ? ` (${new Date(schedule[index]).toISOString().slice(0, 10)})`
                            : '';
        ...
        },
        {
            ...
            task: async (ctx, task) => {
                const page = await buildAboutPage({
                    assetManager: ctx.assetManager,
                    args: ctx.args,
                });
                ...
                try {
                    if (ctx.site.aboutPage) {
                        await ctx.api.pages.edit({
                            id: ctx.site.aboutPage.id,
                            ...page,
                            updated_at: ctx.site.aboutPage.updated_at,
                        });
        ...
        },
        {
            ...
            task: async (ctx, task) => {
                ...
                try {
                    const existing = await ctx.api.posts.browse({
                        filter: 'slug:style-guide',
                        limit: 1,
                    });
        ...
        },
        {
            ...
            task: async (ctx, task) => {
                const parts = {
                    ...
                    author:
                        ctx.args.authorNav !== false && ctx.site.primaryAuthor
                            ? { slug: ctx.site.primaryAuthor.slug }
                            : null,
                    collection:
                        ctx.args.collectionNav !== false
                            ? { tag: (ctx.args.collectionTag || 'lorem').toLowerCase() }
                            : null,
                };
                ...
                try {
                    ...
                    if (error.code === NAV_NO_PERMISSION) {
                        ctx.warnings.push({
                            type: 'navigation',
                            message:
                                'Navigation could not be updated automatically. Integration API keys cannot write settings — re-run with an owner/admin staff access token to enable this, or paste the menu below into Ghost Admin > Settings > Navigation.',
                            navigation: desired,
                        });
                        task.skip(
                            'Integration key cannot write settings (use a staff access token)',
                        );
                    } else {
