...
module.exports = async function (type, ctx) {
    ...
    if ((type === 'posts' || type === 'pages') && (ctx.options.tag || ctx.options.author || ctx.options.visibility)) {
        ...
        if (ctx.options.visibility) {
            filterStringParts.push(`visibility:${ctx.options.visibility}`);
        }
    ...
    if (type === 'tags') {
        if (typeof ctx.options.tag === 'object') {
            ...
            let tagsArray = ctx.options.tag.split(',').map(function (item) {
                return item.trim();
            });
