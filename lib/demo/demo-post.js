...
const makeTitle = (args) => {
    const len =
        Math.floor(Math.random() * (args.titleMaxLength - args.titleMinLength + 1)) +
        args.titleMinLength;
...
const makeParagraphText = (args) => {
    const sentences =
        Math.floor(Math.random() * (args.paragraphUpperBound - args.paragraphLowerBound + 1)) +
        args.paragraphLowerBound;
    return loremIpsum({
        count: sentences,
        units: 'sentences',
