...
const buildDesiredNavigation = ({ current = [], parts = {} }) => {
    ...
    current.forEach((item) => {
        ...
        if (
            managedUrls.has(normalizeNavUrl(item.url)) ||
            managedLabels.has((item.label || '').toLowerCase())
        ) {
            return;
        }
...
const writeNavigation = async ({ options, navigation }) => {
    ...
    const body = {
        settings: [{ key: 'navigation', value: JSON.stringify(navigation) }],
    };
    ...
    if (!response.ok) {
        if (response.status === 403) {
            throw new errors.NoPermissionError({
                message:
                    'This Admin API key cannot write navigation. Use a staff access token (owner/admin) to update settings.',
...
export { NAV_NO_PERMISSION, readSettings, readNavigation, buildDesiredNavigation, writeNavigation };
