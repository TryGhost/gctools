...
const readSettings = async ({options}) => {
    try {
        ...
        const response = await axios.get(settingsUrl(options), {headers});
        const map = {};
...
const buildDesiredNavigation = ({current = [], parts = {}}) => {
    ...
    const managedUrls = new Set(desired.map(item => item.url));
    ...
    current.forEach((item) => {
        ...
        if (managedUrls.has(item.url) || managedLabels.has((item.label || '').toLowerCase())) {
            return;
        }
...
const writeNavigation = async ({options, navigation}) => {
    ...
    try {
        const response = await axios.put(settingsUrl(options), body, {headers});
