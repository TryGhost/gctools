...
const findExistingAuthor = async ({ api, slug, email }) => {
    ...
    return (
        (await tryFilter(`slug:'${escapeFilterValue(slug)}'`)) ||
        (await tryFilter(`email:'${escapeFilterValue(email)}'`))
    );
};
    ...
    if (!response.ok) {
        if (response.status === 403) {
            throw new errors.NoPermissionError({
                message:
                    'This Admin API key cannot create an author. Use a staff access token (owner/admin) to import staff users.',
