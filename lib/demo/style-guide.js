...
const buildStyleGuide = async ({ assetManager, args, site = {}, dummyAuthor = null }) => {
    const children = [];
    const para = (sentences = 2) =>
        L.paragraph([L.textNode(loremIpsum({ count: sentences, units: 'sentences' }))]);
    const section = (title) => children.push(L.heading(title, 'h2'));

    children.push(
        L.paragraph([
            L.textNode(
                'This post exercises every content card available in the Ghost editor, along with a full inline-formatting and typography sampler, so it can be used as a front-end theme test.',
            ),
        ]),
    );
    ...
    children.push(
        L.quote('The way to get started is to quit talking and begin doing. — Walt Disney'),
    );
    children.push(
        L.aside('The only impossible journey is the one you never begin. — Tony Robbins'),
    );
    ...
    children.push(
        L.list(['First ordered item', 'Second ordered item', 'Third ordered item'], {
            ordered: true,
        }),
    );
    ...
    if (assetManager) {
        const regular = await assetManager.getImage({
            key: 'sg-image-regular',
            width: 1200,
            height: 800,
        });
        children.push(
            L.image({
                src: regular.url,
                alt: 'Regular width image',
                caption: 'Regular (in-column) width',
                width: regular.width,
        ...
        const wide = await assetManager.getImage({
            key: 'sg-image-wide',
            width: 1600,
            height: 900,
        });
        ...
        const full = await assetManager.getImage({
            key: 'sg-image-full',
            width: 2000,
            height: 1000,
        });
        ...
        const linked = await assetManager.getImage({
            key: 'sg-image-linked',
            width: 1200,
            height: 800,
        });
        children.push(
            L.image({
                src: linked.url,
                alt: 'Linked image',
                caption: 'This image links to ghost.org',
                width: linked.width,
        ...
        const noCaption = await assetManager.getImage({
            key: 'sg-image-nocaption',
            width: 1200,
            height: 800,
        });
        children.push(
            L.image({
                src: noCaption.url,
                alt: 'Image without a caption',
                width: noCaption.width,
                height: noCaption.height,
            }),
        );

        // Two adjacent images (spacing/margin test).
        const adjA = await assetManager.getImage({
            key: 'sg-image-adj-a',
            width: 1200,
            height: 800,
        });
        const adjB = await assetManager.getImage({
            key: 'sg-image-adj-b',
            width: 1200,
            height: 800,
        });
    ...
    if (assetManager) {
        const galleryImages = [];
        for (let i = 0; i < 6; i += 1) {
            const gi = await assetManager.getImage({
                key: `sg-gallery-${i}`,
                width: 1200,
                height: 800,
            });
    ...
    if (assetManager) {
        ...
        const fileAsset = await assetManager.getFile({
            key: 'sg-file',
        ...
        children.push(
            L.file({
                src: fileAsset.url,
                fileTitle: 'Placeholder document',
                fileCaption: 'A downloadable file card',
    ...
    children.push(
        L.header({
            heading: 'Dark header',
            subheading: 'A full-width header card with a button',
    ...
    children.push(
        L.header({
            heading: 'Light header',
            subheading: 'The same card on a light background',
    ...
    children.push(
        L.header({
            ...
            subheading: 'Using the theme accent colour',
            buttonText: 'Join',
            buttonUrl: '#/portal/signup',
            style: 'accent',
        }),
    ...
    if (assetManager) {
        const headerBg = await assetManager.getImage({
            key: 'sg-header-bg',
            width: 2000,
            height: 1000,
        });
        children.push(
            L.header({
                heading: 'Header with background image',
                subheading: 'Text over a hosted background image',
                buttonText: 'Read more',
                buttonUrl: '#/portal/signup',
                style: 'image',
                backgroundImageSrc: headerBg.url,
                backgroundImageWidth: headerBg.width,
                backgroundImageHeight: headerBg.height,
                size: 'large',
            }),
        );
    }
    ...
    CALLOUT_COLORS.forEach((color) => {
        children.push(
            L.callout(
                `${titleCase(color)} callout — ${loremIpsum({ count: 1, units: 'sentences' })}`,
                { emoji: '💡', backgroundColor: color },
            ),
        );
    });
    children.push(
        L.callout(`Callout with no emoji — ${loremIpsum({ count: 1, units: 'sentences' })}`, {
            emoji: '',
    ...
    children.push(
        L.toggle('When should I use toggles?', loremIpsum({ count: 2, units: 'sentences' })),
    );
    children.push(
        L.toggle(
    ...
    let bookmarkThumb = '';
    if (assetManager) {
        const thumb = await assetManager.getImage({
            key: 'sg-bookmark-thumb',
            width: 1200,
            height: 800,
        });
    ...
    children.push(
        L.bookmark({
            ...
            description:
                'The best open source blog & newsletter platform. Beautiful, contemporary themes for publishers, creators and businesses.',
            publisher: 'Ghost',
    ...
    children.push(
        L.button({ text: 'Centered button', url: '#/portal/signup', alignment: 'center' }),
    );
    children.push(
        L.button({ text: 'Left-aligned button', url: '#/portal/signup', alignment: 'left' }),
    );
    ...
    children.push(
        L.markdown(
            'You can also write in **Markdown**, with _emphasis_, `code`, and [links](https://ghost.org).',
        ),
    );
    ...
    children.push(
        L.html(
            '<div style="padding:1rem;border:1px solid #ddd;border-radius:6px;">Custom HTML card content.</div>',
        ),
    );
    ...
    children.push(
        L.paragraph([
            L.textNode(
                'The following two cards render only in email newsletters and are not visible on the website.',
            ),
        ]),
    );
    ...
    children.push(
        L.heading(
            'A deliberately long heading that should wrap gracefully across multiple lines on narrow viewports without breaking the layout',
            'h3',
        ),
    );
    children.push(
