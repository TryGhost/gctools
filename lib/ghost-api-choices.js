...
import {getMemberLabels, getRoles, getTiers} from './admin-api-call.js';
...
const getAPIAuthorsObj = async () => {
    ...
    apiUsers.forEach(function (author){
        users.push({
            name: `${author.name} ${chalk.grey(`(${author.slug} - ${author.email} - ${author.count.posts} posts)`)}`,
            value: author
        });
    });
...
const getAPIRolesObj = async () => {
    ...
    // Filter out Owner role as it cannot be assigned
