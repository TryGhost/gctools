...
const run = async (argv) => {
    ...
    if (argv.newCanonicalUrl !== null && typeof argv.newCanonicalUrl !== 'string') {
        ui.log.error(
            `--newCanonicalUrl requires an argument. Omit the flag entirely to clear canonical_url.`,
        );
        return;
    }
    ...
    if (argv.dateRange === 'custom') {
        if (!argv.dateRangeStart || !argv.dateRangeEnd) {
            ui.log.error(
                `--dateRange=custom requires both --dateRangeStart and --dateRangeEnd (YYYY-MM-DD).`,
            );
            return;
        }
