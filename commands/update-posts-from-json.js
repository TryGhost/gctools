...
const paramsDesc = ['URL to your Ghost API', 'Admin API key', 'Path to the Ghost export JSON file'];
...
const run = async (argv) => {
    ...
    if (argv.fields) {
        ...
        if (invalidFields.length > 0) {
            ui.log.error(
                `Invalid fields: ${invalidFields.join(', ')}. Available fields: ${validFieldValues.join(', ')}`,
            );
            return;
        }
        ...
        if (fields.length === 0) {
            ui.log.error(
                `No valid fields provided. Available fields: ${validFieldValues.join(', ')}`,
            );
            return;
        }
    } else {
